...
class GozarProfile {
  ...
  factory GozarProfile.fromJson(Map<String, dynamic> input) => GozarProfile(
    input['name']?.toString() ?? 'سرور شخصی',
    input['link']?.toString() ?? '',
  );
...
class _GozarHomeState extends State<GozarHome> with WidgetsBindingObserver {
  ...
  bool get canDisconnect =>
      busy ||
      stopping ||
      const ['running', 'starting', 'consent', 'stopping'].contains(stage);
  ...
  void initState() {
    ...
    mode = widget.preferences.getString('gozar_routing_mode') == 'selected'
        ...
        : 'all';
    packages =
        (widget.preferences.getStringList('gozar_selected_packages') ??
                const <String>[])
            .toSet();
  ...
  Future<void> saveProfile({bool quiet = false}) async {
    ...
    try {
      ...
      if (index >= 0 && index < updated.length) {
        ...
        updated.add(
          GozarProfile('سرور ' + (updated.length + 1).toString(), input),
        );
      }
      if (!mounted) return;
      setState(() {
  ...
  Future<void> sampleCounters() async {
    ...
    try {
      ...
      final download = rx >= receivedAtSample!
          ? (rx - receivedAtSample!) * 8 / (seconds * 1000000)
          : 0.0;
      final upload = tx >= sentAtSample!
          ? (tx - sentAtSample!) * 8 / (seconds * 1000000)
          : 0.0;
      setState(() {
        receivedMbps = download;
        sentMbps = upload;
        receivedSession = rx >= receivedAtStart! ? rx - receivedAtStart! : 0;
  ...
  Future<void> checkTcpLatency() async {
    ...
    try {
      ...
      if (mounted)
        setState(() {
          tcpLatencyMs = timer.elapsedMilliseconds;
        });
    } catch (_) {
  ...
  Future<void> refresh() async {
    ...
    final expectedOperation = operation;
    try {
      ...
      setState(() {
        ...
        detail = switch (status) {
          'running' => 'تونل VPN اندروید فعال است.',
          'starting' => 'در حال راه‌اندازی موتور Xray و تونل اندروید…',
          'stopping' => 'در حال بستن تونل VPN…',
          'consent' => 'مجوز VPN را در پنجره سیستم تأیید کنید.',
          'error' =>
            'موتور VPN راه‌اندازی نشد؛ کانفیگ و مجوز اندروید را بررسی کنید.',
          _ => 'VPN خاموش است.',
        };
      });
    } catch (_) {
      if (mounted &&
          ...
          expectedOperation == operation) {
        setState(() {
          detail = 'سرویس VPN در دسترس نیست.';
        });
      }
    }
  }
  ...
  Future<void> connect() async {
    ...
    try {
      ...
      final routingPackages = packages.toList();
      await _vault.write(key: 'gozar_xray_profile', value: input);
      // A cancel during secure storage must never start a late VPN.
      if (!mounted || attempt != operation) return;
      await SystemVpnBridge.start(
        config,
        mode: routingMode,
        packages: routingPackages,
      );
      if (!mounted || attempt != operation) return;
      await refresh();
      // Continuous native polling also covers permission dialogs, later
      // service errors, and disconnects made through Android settings.
    } on FormatException catch (error) {
      if (attempt == operation) notice(error.message.toString());
    } catch (_) {
      if (mounted && attempt == operation) {
  ...
  Future<void> disconnect() async {
    if (stopping) return;
    ++operation; // Invalidate any pending connect, regardless of busy.
    setState(() {
      stopping = true;
      busy = false;
    });
    ...
    try {
      await showModalBottomSheet<void>(
        ...
        builder: (sheet) => StatefulBuilder(
          builder: (sheet, updateSheet) {
            final visible = installed
                .where(
                  (app) =>
                      (app['label'] ?? '').toLowerCase().contains(search) ||
                      (app['package'] ?? '').toLowerCase().contains(search),
                )
                .toList();
            return SafeArea(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text(
                      'کدام برنامه‌ها از گذر استفاده کنند؟',
                      style: TextStyle(
                    ...
                    const Text(
                      'نبض خبر و کافی‌نت نیز مانند بقیه برنامه‌های گوشی '
                    ...
                    if (choice == 'selected') ...[
                      ...
                      Text(
                        picked.length.toString() + ' برنامه انتخاب شده',
                    ...
                    FilledButton(
                      onPressed: choice == 'selected' && picked.isEmpty
                          ...
                          : () async {
                              ...
                              if (stage == 'running') {
                                notice(
                                  'برای اعمال انتخاب جدید، VPN را قطع و دوباره وصل کنید.',
                                );
                              }
                            },
  ...
  Widget _hero({bool compact = false, bool tight = false}) {
    ...
    final transitioning =
        busy ||
        stopping ||
        const ['starting', 'consent', 'stopping'].contains(stage);
    return GozarPanel(
      padding: EdgeInsets.symmetric(vertical: compact ? 8 : 14, horizontal: 14),
      glow: connected ? GozarPalette.cyan : GozarPalette.blue,
      child: Column(
        children: [
          GozarPowerButton(
            key: const ValueKey('gozar-power'),
            diameter: compact ? 148 : 184,
            connected: connected,
            busy: transitioning,
            label: stopping || stage == 'stopping'
                ? 'در حال قطع اتصال'
                : canDisconnect
          ...
          Text(
            detail,
            key: const ValueKey('gozar-vpn-status'),
            textAlign: TextAlign.center,
            style: TextStyle(
              color: connected ? GozarPalette.cyan : GozarPalette.muted,
              fontSize: 12,
            ),
          ),
          ],
  ...
  Widget _connectActions() => Row(
    children: [
      Expanded(
        child: FilledButton.icon(
          key: const ValueKey('gozar-connect'),
          onPressed: canDisconnect ? null : connect,
      ...
      Expanded(
        child: OutlinedButton.icon(
          ...
          onPressed: stopping || !canDisconnect ? null : disconnect,
  ...
  Widget _profileInput() => GozarPanel(
    ...
    child: Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _eyebrow(Icons.link_rounded, 'کانفیگ سرور', color: GozarPalette.purple),
        const SizedBox(height: 7),
        const Text(
          'VMess / VLESS / Trojan / Xray JSON',
          textDirection: TextDirection.ltr,
        ...
        OutlinedButton.icon(
          ...
          label: Text(
            selectedProfile < 0 ? 'ذخیره سرور جدید' : 'ذخیره تغییرات سرور',
          ),
          style: OutlinedButton.styleFrom(
  ...
  Widget _traffic() => GozarPanel(
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 6),
        const Text(
          'برآورد بر اساس ترافیک UID برنامه گذر؛ نه سرعت '
        ...
        if (receivedSeries.isEmpty && sentSeries.isEmpty)
          const SizedBox(
            ...
            child: Center(
              child: Text(
                'پس از اتصال و تبادل داده، نمودار ظاهر می‌شود.',
                textAlign: TextAlign.center,
        ...
        Row(
          children: [
            GozarMetric(
              ...
              caption: 'دریافت تقریبی',
              value: _speed(receivedMbps),
            ),
            const SizedBox(width: 8),
            GozarMetric(
              icon: Icons.north_rounded,
              caption: 'ارسال تقریبی',
              value: _speed(sentMbps),
              accent: GozarPalette.purple,
            ),
          ],
        ),
        const SizedBox(height: 8),
        Row(
          children: [
            GozarMetric(
              ...
              caption: 'دریافت در این نشست',
              value: _volume(receivedSession),
            ),
            const SizedBox(width: 8),
            GozarMetric(
              icon: Icons.upload_rounded,
              caption: 'ارسال در این نشست',
              value: _volume(sentSession),
              accent: GozarPalette.purple,
            ),
          ],
        ),
            ...
            Expanded(
              child: Text(
                connectedObservedAt == null
                    ...
                    : 'مدت نمایش اتصال: ' +
                          DateTime.now()
                              .difference(connectedObservedAt!)
                              .inMinutes
                              .toString() +
                          ' دقیقه',
  ...
  Widget _tcpTest() => GozarPanel(
    glow: GozarPalette.purple,
    child: Column(
      children: [
        _eyebrow(
          Icons.speed_outlined,
          'آزمایش دسترسی به سرور',
        ...
        const SizedBox(height: 6),
        const Text(
          'این عدد زمان اتصال مستقیم TCP به سرور است، '
          'نه پینگ از داخل VPN.',
          style: TextStyle(color: GozarPalette.muted, fontSize: 11),
        ),
        TextButton.icon(
  ...
  Widget _home() => LayoutBuilder(
    ...
    builder: (context, constraints) {
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'مصرف نشست: ' +
                  _volume(
                    receivedSession == null || sentSession == null
                        ? null
                        : receivedSession! + sentSession!,
                  ),
              style: const TextStyle(color: GozarPalette.text, fontSize: 12),
            ),
            const SizedBox(height: 4),
            const Text(
              'ترافیک تقریبی برنامه گذر',
              style: TextStyle(color: GozarPalette.muted, fontSize: 10),
            ),
          ],
        ),
      ...
      return Padding(
        ...
        child: constraints.maxWidth > 600 && constraints.maxHeight < 500
            ? Row(
            ...
            : Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [hero, const SizedBox(height: 10), metrics],
              ),
      );
    },
  );
      ...
      child: Column(
        children: [
          Row(
            children: [
              Icon(
                selected ? Icons.check_circle_rounded : Icons.dns_rounded,
      ...
      GozarPanel(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'سرورها و لینک‌ها فقط از خودتان دریافت می‌شوند؛ '
      ...
      GozarPanel(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'انتخاب برنامه‌های VPN، امنیت و کنترل وضعیت اتصال',
              style: TextStyle(color: GozarPalette.muted, fontSize: 12),
            ),
            const SizedBox(height: 12),
            OutlinedButton.icon(
            ...
            const SizedBox(height: 8),
            const Text(
              'در صورت تغییر برنامه‌ها، VPN را قطع و دوباره وصل کنید.',
              style: TextStyle(color: GozarPalette.muted, fontSize: 11),
            ),
            TextButton.icon(
              onPressed: refresh,
      ...
      GozarPanel(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 11),
            const Text(
              'Kill Switch از طریق تنظیمات رسمی اندروید',
              style: TextStyle(
                color: GozarPalette.text,
                fontSize: 15,
            ...
            const SizedBox(height: 8),
            const Text(
              'در تنظیمات VPN اندروید، گذر را انتخاب کنید '
      ...
      GozarPanel(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            const Text(
              'گذر تاریخچه وب‌گردی را در رابط خود ذخیره نمی‌کند. '
            ...
            const SizedBox(height: 10),
            const Text(
              'وضعیت «تونل فعال» به‌معنی تایید سرعت یا '
