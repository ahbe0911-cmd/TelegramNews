...
parser.add_argument('variant', choices=['news', 'cafenet'])
args = parser.parse_args()
root = Path(__file__).resolve().parents[1]
cafe = args.variant == 'cafenet'
package = 'ir.channel.telegram_cafenet' if cafe else 'ir.channel.telegram_tdnews'
label = 'کافی‌نت' if cafe else 'نبض خبر'
folder = 'Cafenet' if cafe else 'NabzKhabar'
...
color = '#7043C6' if cafe else '#0866DC'
